...
router.get("/", async (req, res) => {
  try {
    ...
    if (!allproducts.length) {
      return res
        .status(404)
        .json({ state: false, msg: "Data is not defined", innerData: [] });
    }
    res
      .status(200)
      .json({ state: true, msg: "All products", innerData: allproducts });
  } catch {
    res
      .status(500)
...
router.post("/search", async (req, res) => {
  try {
    ...
    const find = await allProducts.filter((i) =>
      i.title.toLowerCase().includes(productName.toLowerCase())
    );
    if (!find.length) {
      return res.json({
        state: false,
        mag: "Malumotlat topilmadi",
        innerData: null,
      });
    }
    res
      .status(200)
      .json({ state: true, mag: "Topilgan ma'lumotlar", innerData: find });
  } catch (err) {
    res
      .status(500)
      .json({ state: false, msg: "server error", innerData: null });
...
router.get("/single/:_id", async (req, res) => {
  try {
    ...
    if (!product) {
      return res.json({
        state: false,
        msg: "Product with unique id found",
        innerData: null,
      });
    }
    res.json({
